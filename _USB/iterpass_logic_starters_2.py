...
# How often to force a hardware-level sync (in lines)
...
def generate_targeted_list():
    ...
    # ---------------------------------------------------------
    # BUFFERING NOTE: Set buffering=1 to flush Python's buffer.
    # We will manually call os.sync() to flush the OS buffer.
                ...
                for r in range(MIN_ADDITIONS, MAX_ADDITIONS + 1):
                    if r == 0:
                        ...
                        for addition in itertools.product(fillers, repeat=r):
                            ...
                            # Memory Management: Force OS to empty the RAM cache
                            lines_since_sync += 1
                            if lines_since_sync >= SYNC_INTERVAL:
                                os.sync() # Hard flush to SSD
                                lines_since_sync = 0
